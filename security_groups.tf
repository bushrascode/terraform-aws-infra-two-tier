...
resource "aws_security_group" "securitygroup_loadbalancer" {
  name        = "vpc security group for load balancer"
  description = "security group for load balancer"
  vpc_id      = aws_vpc.main.id

  tags = {
    Name = "security_group_load_balancer"
  }
  // ingress rule
...
resource "aws_security_group" "db_securitygroup" {
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.security_group_web_access.id, aws_security_group.security_group_ssh_access.id]
  }
